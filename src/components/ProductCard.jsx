...
export default function ProductCard({ product: { title: name, _id: id, main_image, price, discount_price, discount: isSale } }) {
    ...
    return (
        <div className="max-w-[300px] w-full border-t-2 border-t-transparent hover:border-t-[#46A358] transi group rounded">
            <div className="card_img relative transi rounded overflow-hidden ">
                ...
                {isLiked && <div className={`absolute transi group-hover:opacity-0 group-hover:-top-5 group-hover:-right-10 ${isInCart ? 'opacity-0' : 'opacity-100'} top-0 right-0 bg-[#46A358] text-white text-sm px-2 py-1 font-bold`}>In your Wishlist</div>}
                {isInCart && <div className={`absolute opacity-100 transi group-hover:opacity-0 group-hover:-top-5 group-hover:-right-10 top-0 right-0 bg-[#46A358] text-white text-sm px-2 py-1 font-bold`}>In your Cart</div>}
                {isSale && <div className={`absolute opacity-100 transi group-hover:opacity-0 group-hover:-left-15 group-hover:-top-5 top-0 left-0 bg-[#46A358] text-white px-2 py-[2px] font-bold`}>{discountPercent}% OFF</div>}
            </div>
            <div>
